...
std::optional<std::chrono::microseconds> micros_until(
...
VescClient::SubscriptionHandle& VescClient::SubscriptionHandle::operator=(SubscriptionHandle&& other) noexcept {
    if (this != &other) {
        reset();
        unsubscribe_ = std::move(other.unsubscribe_);
    }
...
std::optional<VescClient::ScheduledRequest> VescClient::make_due_poll_request(
    PollChannel& channel,
    const SteadyClock::time_point& now) {
    const auto interval_ms = channel.interval_ms.load();
    if (interval_ms <= 0) {
    ...
    ScheduledRequest request;
    request.expected_id = static_cast<std::uint8_t>(
        channel.kind == PollChannel::Kind::Imu ? VescPacketCommID::GetImuData : VescPacketCommID::GetValues);
...
std::optional<VescClient::ScheduledRequest> VescClient::dequeue_ready_query(
    const SteadyClock::time_point& now) {
    std::lock_guard lock(scheduler_mutex_);
    while (!request_queue_.empty() && request_queue_.front().deadline <= now) {
        auto expired = std::move(request_queue_.front());
