...
VescPacketBuilder::VescPacketBuilder() : buffer_() {}

VescPacketBuilder::VescPacketBuilder(std::size_t buffer_size) : buffer_() {
    buffer_.reserve(buffer_size);
}


template <typename T>
void append_integral_be(std::vector<std::uint8_t>& buffer, T value) {
    
    static_assert(std::is_integral_v<T>,
                  "append_integral_be requires an integral type");
...
std::vector<std::uint8_t> VescPacketBuilder::build_packet(const std::vector<Field>& fields) {
    ...
    for (const auto& field : fields) {
        std::visit([this](const auto& arg) {
            append_integral_be(buffer_, arg);
        }, field);
    }
