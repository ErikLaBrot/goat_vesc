# goat_motor_controller AI Context

This is the entry point for coding agents changing `goat_motor_controller`. It supplies
repository context and routes readers to the existing API and operator
documentation rather than repeating it.

## Project Boundary

`goat_motor_controller` is a C++17 library that owns one controller serial
transport, translates between typed values and firmware packets, schedules telemetry and diagnostic
...
- controller firmware, operator calibration policy, or schema-level interpretation
...
`goat_motor_controller` independently implements the required subset of the
VESC serial `COMM_*` protocol defined by the upstream
...
|---|---|---|
| Public contract | Consumer-facing configuration, data types, lifecycle, control, telemetry, and query APIs | [`include/goat_motor_controller/`](../../include/goat_motor_controller/) |
...
| Message protocol | Command IDs, field masks, scaling, and typed request/response encoding | [`protocol_ids.hpp`](../../include/goat_motor_controller/protocol_ids.hpp), [`protocol.cpp`](../../src/protocol.cpp) |
| Transport and scheduling | Serial lifecycle, work arbitration, timeouts, reply matching, caches, callbacks, and shutdown | [`controller_client.cpp`](../../src/controller_client.cpp) |
...
| Periodic telemetry | IMU and motor-state poll channels become due independently. The scheduler sends a poll when no reply-bearing request is in flight, then the I/O thread decodes and timestamps the reply, updates the latest-value cache, and publishes callbacks outside internal locks. Firmware-7 motor state includes command-timeout and kill-latch status when the full reply tail is present. IMU wins a tie between due channels. |
...
- One active process owns a controller transport; inside `ControllerClient`, one I/O
...
- The optional host watchdog is one-shot and disabled by default. It discards
  ...
  after transport loss. Firmware-side timeout configuration remains the backstop.
- Repository automation must not touch real controller hardware. Hardware access and
  actuator commands require explicit authorization.

## Sources Of Truth

- Public contract: [`include/goat_motor_controller/`](../../include/goat_motor_controller/)
