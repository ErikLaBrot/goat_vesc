...
int main(int argc, char** argv) {
    ...
    auto imu_sub = client.subscribe_imu([](const VescIMUData& imu) {
        std::cout << "IMU stamp_ns=" << imu.stamp_ns
                  << " gyro_z=" << imu.gyro_z
                  << " acc_x=" << imu.acc_x << '\n';
    });

    auto motor_sub = client.subscribe_motor_state([](const VescMotorState& state) {
        std::cout << "Motor stamp_ns=" << state.stamp_ns
                  << " rpm=" << state.rpm
                  << " vin=" << state.vin << '\n';
    });

    if (auto fw = client.request_fw_version(200ms)) {
        std::cout << "Firmware " << static_cast<int>(fw->major)
    ...
    if (auto latest = client.latest_motor_state()) {
        std::cout << "Latest cached rpm=" << latest->rpm
