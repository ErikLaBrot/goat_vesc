...
void print_usage(const char* argv0) {
  std::cout << "Usage: " << argv0 << " [device_path] [baud] [center] [amplitude]\n"
...
int main(int argc, char** argv) {
  try {
    ...
    std::cout << "Servo center=" << center << " left=" << left << " right=" << right << '\n';
    ...
    const std::vector<float> sweep{
        center, left, center, right, center, left, center, right, center,
    };
